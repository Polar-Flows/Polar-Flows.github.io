...
class PolarFlowsApp {
  ...
  async init() {
    try {
      ...
      // Initialize dynamic team grid layout
      this.initTeamGridLayout();
  ...
  initSlidingBackground() {
    ...
    if (isSubPage) {
      this.debugLog('Sub-page detected (contact/privacy) - NO background JavaScript will run, CSS only');
      return; // Exit immediately - no background JavaScript for sub-pages
    }
    
    // Only run background JavaScript for main page
    this.debugLog('Main page detected - initializing background system');
    ...
    this.debugLog('iOS Detection: ' + isIOS + ' User Agent: ' + navigator.userAgent);
    
    if (isIOS) {
      // For main page on iOS: Use color background with image fade-in
      this.debugLog('iOS main page detected - calling initIOSBackground');
  ...
  initIOSBackground(hero) {
    this.debugLog('=== iOS BACKGROUND INITIALIZATION STARTED ===');
    this.debugLog('Initializing iOS background with smooth transition...');
    ...
    this.debugLog('Background element inserted into document body');
    ...
    this.debugLog('Background clipped to hero section: ' + clipPath);
    ...
    if (heroContent) {
      ...
      this.debugLog('Hero content z-index set to 10');
    }
    
    // Also ensure hero itself has proper positioning
    hero.style.position = 'relative';
    hero.style.zIndex = '5';
    this.debugLog('Hero section positioning set');
    ...
    if (testImg.complete && testImg.naturalHeight !== 0) {
      this.debugLog('iOS background image already cached - showing immediately');
    ...
    testImg.onload = () => {
      this.debugLog('iOS background image loaded successfully, starting smooth transition');
      this.smoothTransition(backgroundElement, hero);
    };
    testImg.onerror = () => {
      this.debugLog('iOS background image failed to load - keeping color background');
    };
    this.debugLog('Loading image from: ' + testImg.src);
    
    this.debugLog('iOS background initialized with color fallback');
  }
  ...
  smoothTransition(backgroundElement, hero) {
    ...
    const fadeOutInterval = setInterval(() => {
      ...
      if (opacity <= 0) {
        ...
        this.debugLog('Smooth transition completed');
      }
  ...
  /**
   * Initialize dynamic team grid layout based on number of team members
   */
  initTeamGridLayout() {
    ...
    teamGrid.classList.remove('layout-4-members', 'layout-5-members', 'layout-6-members', 'layout-7-members', 'layout-8-members');

    // Apply appropriate layout class based on member count
    if (memberCount >= 5) {
      teamGrid.classList.add(`layout-${memberCount}-members`);
    }

    this.debugLog(`Team grid initialized with ${memberCount} members`);
  }
    ...
    if ('serviceWorker' in navigator) {
      window.addEventListener('load', () => {
        navigator.serviceWorker.register('/sw.js')
          .then(registration => {
            this.debugLog('Service Worker registered successfully: ' + registration.scope);
            ...
            registration.addEventListener('updatefound', () => {
              ...
              newWorker.addEventListener('statechange', () => {
                if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                  this.debugLog('New service worker available. Refresh to update.');
          ...
          .catch(error => {
            this.debugLog('Service Worker registration failed: ' + error);
          });
      });
    } else {
      this.debugLog('Service Worker not supported in this browser');
    }
  }
  ...
  measurePageLoad() {
    if ('performance' in window) {
      ...
      if (navigation) {
        ...
        // Page load metrics logged (debug logging disabled)
      }
    }
  }
